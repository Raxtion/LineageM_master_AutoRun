...
for index in Train_Index:
    os.chdir('C:\\Users\\raxku\\OneDrive\\Documents\\FunctionLib\\python\\pyCode\\MachineLearning_leaning\\LetterRecognition\\KNN\\picture\\' + str(index))
    ...
    for file in file_list:
        ...
        img = cv2.imread(file)
        #print("img=", img.shape)
        gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)     #type = np.array((15x10), 'uint8')
        #print("img=", gray.shape)
        data = gray.reshape(1, 15*10).astype('float32') #type = np.array((150), 'float32')
...
np.save('trains_list.npy', trains_list)
